...
original_label_dict = {0: "unknown", 1: "field"}
remap_label_dict = {0: 0, 1: 1}
...
def TestSentinel2_transform(model_config, data_config, is_training):
    """
    :param npixel:
    :return:
    """
    ...
    transform_list.append(Concat(concat_keys=['x10', 'x20', 'x60', 'day']))     # concat x10, x20, x60, day, year
    if bidir_input:
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        if 'B01' in sample.keys():
            ...
            return sample
        # else:
        sample['x10'] = torch.tensor(sample['x10'].astype(np.float32)).type(torch.float32)
        sample['x20'] = torch.tensor(sample['x20'].astype(np.float32)).type(torch.float32)
        sample['x60'] = torch.tensor(sample['x60'].astype(np.float32)).type(torch.float32)
        sample['day'] = torch.tensor(sample['day'].astype(np.float32)).type(torch.float32)
        sample['year'] = torch.tensor(sample['year'].astype(np.float32)).type(torch.float32)
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        sample['year'] = (sample['year'] - Normalize.YEAR_BEGIN) / (Normalize.YEAR_END - Normalize.YEAR_BEGIN)
        return sample
